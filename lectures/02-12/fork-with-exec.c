...
#if 0
    execl( "/bim/ls", "ls",    "-l",    NULL );  <== typo in path

    execl( "/bin/ls", "ls", "-l", "nosuchfile.txt", NULL );  <== ls error

    TO DO: try running different commands
#endif

    /* we should NEVER get to this point of the code
       (unless execl() failed) */

    perror( "execl() failed" );

#if 0
    /* this execlp() call will search for "ls" as an executable file
        within the $PATH environment variable (just like the shell) */
    execlp( "ls", "ls",    "-l",    NULL );
#endif
